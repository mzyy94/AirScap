...
sequenceDiagram
    ...
    loop 各ページ (256KB チャンク転送)
        ...
        C->>S: ADF ステータス確認 (GET_STATUS cmd=0x0A)
    end
...
| +11 | 1 | Bleed-through | `0xC0`=裏写り防止ON, `0x80`=OFF | |
| +12 | 1 | 定数 | `0x80` | |
...
スキャン中のステータスを確認する。ADF（自動原稿送り装置）の用紙有無を判定するために使用される。

ADF ステータスはオフセット 44 で確認する。プレスキャンチェック・スキャンループ中のいずれも同一オフセットを参照する。
...
| 4 | 36 | Header | ゼロ埋め |
| 40 | 4 | Scan Status | スキャン進行状態 |
| 44 | 4 | ADF Status | 用紙有無（ビットマスクで判定） |
| 48 | 8 | Reserved | |
| 56 | 4 | Device Flags | デバイス状態フラグ（用紙検出には使用しない） |
| 60 | 12 | Reserved | ゼロ埋め |

**ADF 用紙検出:**

ビットマスク `0x00010000` で用紙の有無を判定する:
...
用紙がない場合、クライアント側でエラーを生成して中断する（プロトコル上のエラーコードは存在しない）。

> **注:** オフセット 56 の値（例: `0xC0000000`）はデバイス状態フラグであり、用紙が装填されていても `0x00010000` ビットがセットされない場合がある。用紙検出には必ずオフセット 44 を使用すること。
...
sequenceDiagram
    ...
    S->>C: ACK
    C->>S: GET_STATUS(0x0A) 用紙チェック
    S->>C: ADF ステータス

    Note over C,S: スキャン開始待機
    C->>S: GET_SET(sub=0xE0) スキャン開始待機
    S->>C: ACK（ボタン押下 or トリガー後に応答）
    ...
    loop 各物理シート
        ...
        C->>S: GET_STATUS(0x0A) 次シート有無チェック
        S->>C: ADF ステータス
        opt 用紙あり
            C->>S: GET_SET(sub=0xE0) 次シート待機
            S->>C: ACK (status=0: 続行, status≠0: 完了)
        end
    end
```
...
stateDiagram-v2
    ...
    ScanSession --> PaperCheck: ADF 用紙チェック
    PaperCheck --> Error: 用紙なし
    PaperCheck --> Scanning: 用紙あり
    Scanning --> PageTransfer: チャンク転送
    PageTransfer --> Scanning: 次ページあり
...
1. **Page Flags** — 奇数/偶数シートで `0x00800400` / `0x00000400` が切り替わる理由（給紙方向に関連すると推測）
2. **エラーハンドリング** — 紙詰まりやマルチフィード検出時のプロトコル動作。用紙なしはクライアント側で ADF ステータスビットから判定するが、スキャナー側からのエラー通知は未定義
3. **Config Data の一部定数** — +9: `0xC8`, +12: `0x80`, +31: `0x30`, +50: `0x04`, +54〜+56: `0x010101` の正確な意味
